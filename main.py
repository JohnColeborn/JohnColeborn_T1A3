        ...
        if choice == '1':
            ...
            while life >= 1:  # set lives based on life to countdown
                ...
                if len(first_guess) > 1:
                    if first_guess == word:  # if user guesses word correctly
                        ...
                        break
                elif first_guess in word:  # Correct Character Guess
                    guessed_word = [first_guess if c == first_guess else gw for c, gw in zip(word, guessed_word)]  # Updates guessed word based on correct guess
                    if ''.join(guessed_word) == word:  # If word is guessed by char completion
                        print(f"Wow, didn't expect you to guess {word} so easily! You brainiac you!")
                        break
                    else:
